...
# Compiler
ps2exe -InputFile "Xpolaris-GUI.ps1" -OutputFile "Xpolaris-GUI.exe" -NoConsole -requireAdmin -Title "Windows Customizer by Xpolaris" -Version "4.3.2"
```
...
### v4.3.2 (15 fevrier 2026)
- **FIX CRITIQUE** : Logique contradictoire du branding OEM resolue
  - Si "Supprimer branding OEM" coche, le branding Xpolaris ne sera JAMAIS ajoute (exclusion mutuelle)
  - Garantit un systeme professionnel 100% neutre sans pollution de branding
- **FIX** : Suppression complete des stub shortcuts Windows 11 (OneDrive, Outlook, LinkedIn)
  - RemoveBloatware.ps1 supprime desormais les fichiers SANS extension (stubs modernes)
  - En plus des .lnk classiques, elimine les entrees de la base de donnees du menu Demarrer
- **Amelioration** : Messages de log plus precis dans RemoveBloatware.ps1 (distinction .lnk vs stubs)

### v4.3.1 (15 fevrier 2026)
- **FIX CRITIQUE** : Scripts PowerShell desormais executables manuellement
  - Ajout de `Unblock-File` apres chaque copie de .ps1 (6 emplacements)
  - Supprime le marqueur NTFS `Zone.Identifier` qui bloque l'execution
- **NOUVEAU** : Lanceurs .cmd pour execution sans ExecutionPolicy
  - `RemoveBloatware.cmd` : Lance le script de suppression du bloatware
  - `RemoveOEMBranding.cmd` : Lance le script de suppression du branding OEM
  - Verification des droits admin + Contournement ExecutionPolicy Restricted
- **Benefice** : Re-execution manuelle possible depuis `$OEM$\Scripts` (scenarios hors ligne)
- **PSScriptAnalyzer** : Reduction de 914 avertissements a ~0 (exclusions justifiees)
